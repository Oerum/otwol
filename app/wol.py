...
ping_timeout = os.environ.get('SCAN_PING_TIMEOUT_MS', 300)
...
enable_login = os.environ.get('APP_LOCAL_AUTH', 'false').strip().lower() == 'true'

db_path = os.environ.get('DATABASE_FILE') or '/app/db/computers.db'
...
oidc_enabled = os.environ.get('APP_OIDC_AUTH', 'false').strip().lower() == 'true'
oidc_issuer = os.environ.get('OIDC_ISSUER_URL', '')
oidc_client_id = os.environ.get('OIDC_CLIENT_ID', '')
oidc_client_secret = os.environ.get('OIDC_CLIENT_SECRET', '')
oidc_redirect_uri = os.environ.get('OIDC_REDIRECT_URL', '').rstrip('/') + '/auth/oidc/callback'
...
username = os.environ.get('APP_USER', 'admin').strip('"')
password = os.environ.get('APP_PASS', 'admin').strip('"')
...
class Computer(db.Model):
  ...
  schedules_paused = db.Column(db.Boolean, default=False, nullable=False)
  excluded_days = db.Column(db.String(32), default='', nullable=False)
  vacation_until = db.Column(db.String(10), nullable=True)
  cron_wol_until = db.Column(db.String(10), nullable=True)
  cron_sol_until = db.Column(db.String(10), nullable=True)
...
def load_computers():
  ...
  for c in Computer.query.all():
    computers.append({
      ...
      'schedules_paused': c.schedules_paused,
      'excluded_days': c.excluded_days,
      'vacation_until': c.vacation_until,
      'cron_wol_until': c.cron_wol_until,
      'cron_sol_until': c.cron_sol_until
    })
  ...
  try:
    ...
    for line in lines:
      ...
      if len(fields) >= 7:
        ...
        # If the command still points to wakeonlan, migrate it
        if '/usr/local/bin/wakeonlan' in command:
          mac_address = command.split()[-1]
          new_command = f"{schedule} {user} /usr/local/bin/python /app/cron_trigger.py {mac_address}"
    ...
    if modified:
      ...
      logger.info("Successfully migrated existing cron entries to run cron_trigger.py wrapper")
  except Exception as e:
    logger.error(f"Error migrating cron file: {e}")
...
def add_cron(mac_address, request_cron, request_until=None):
  messages = []
  # Check Entries
  ...
  cron_command = f"{request_cron} root /usr/local/bin/python /app/cron_trigger.py {mac_address}"
  ...
  if not computer:
    reversed_mac = ':'.join(reversed(mac_address.split(':')))
  ...
  if computer:
    if is_sol:
      computer.cron_sol_until = request_until
    else:
      computer.cron_wol_until = request_until
    db.session.commit()
...
def add_wol_cron():
  ...
  request_until = request.form.get('cron_until') or None
  return add_cron(request_mac_address, request_cron, request_until)
...
def add_sol_cron():
  ...
  request_cron = request.form['cron_request']
  request_until = request.form.get('cron_until') or None
  return add_cron(reversed_mac_address, request_cron, request_until)
...
def delete_wol_cron():
  ...
  computer = Computer.query.filter_by(mac_address=request_mac_address).first()
  if computer:
    computer.cron_wol_until = None
    db.session.commit()
  return redirect(url_for('wol_form'))
...
def update_schedule_settings():
  mac_address = request.form['mac_address']
  schedules_paused = 'schedules_paused' in request.form
  excluded_days = ','.join(request.form.getlist('exclude_day'))
  vacation_until = request.form.get('vacation_until') or None
  ...
  if vacation_until:
    try:
      ...
      datetime.strptime(vacation_until, '%Y-%m-%d')
    except ValueError:
      flash("Invalid vacation date format. Use YYYY-MM-DD.", "danger")
      return redirect(url_for('wol_form'))

  computer.schedules_paused = schedules_paused
  computer.excluded_days = excluded_days
  computer.vacation_until = vacation_until
  db.session.commit()

  flash(f"Schedule settings for {computer.name} updated.", "success")
  return redirect(url_for('wol_form'))
...
with app.app_context():
  ...
  try:
    ...
    with db.engine.begin() as conn:
      if 'schedules_paused' not in columns:
        conn.execute(db.text("ALTER TABLE computer ADD COLUMN schedules_paused BOOLEAN DEFAULT 0 NOT NULL"))
      if 'excluded_days' not in columns:
        conn.execute(db.text("ALTER TABLE computer ADD COLUMN excluded_days VARCHAR(32) DEFAULT '' NOT NULL"))
      if 'vacation_until' not in columns:
        conn.execute(db.text("ALTER TABLE computer ADD COLUMN vacation_until VARCHAR(10) NULL"))
      if 'cron_wol_until' not in columns:
        conn.execute(db.text("ALTER TABLE computer ADD COLUMN cron_wol_until VARCHAR(10) NULL"))
      if 'cron_sol_until' not in columns:
        conn.execute(db.text("ALTER TABLE computer ADD COLUMN cron_sol_until VARCHAR(10) NULL"))
  except Exception as e:
